...
public class BiometricLogger {
    ...
    public BiometricLogger(Context ctx) {
        this.ctx = ctx;
    }

    public void startLogging() {
        isLogging = true;
        // TODO: Integrate with biometric API and log events
    }
    ...
    // Call this when a biometric event occurs
    public void logEvent(String eventType) {
        if (isLogging) {
            long ts = System.currentTimeMillis();
            String raw = eventType + "_" + ts;
            String hashed = HashUtil.hash(raw);
            CSVWriter.append("biometric", hashed, ts);
        }
    }
}
